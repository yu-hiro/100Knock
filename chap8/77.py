...
# 損失を計算
def cal_loss(net, loader_data, criterion):
...
# 学習
def train_net(ds_train, ds_valid, net, optimizer, criterion, batch):
    # DataLoaderを作成
    ...
    for epoch in range(epochs):
        ...
        # 損失、正解率を計算
        train_loss = cal_loss(net, loader_train, criterion)
        train_accuracy = cal_acc(net, loader_train)
        valid_loss = cal_loss(net, loader_valid, criterion)
        valid_accuracy = cal_acc(net, loader_valid)
...
def main():
    ...
    for batch in batch_size:
        train_net(ds_train, ds_valid, net, optimizer, criterion, batch)
